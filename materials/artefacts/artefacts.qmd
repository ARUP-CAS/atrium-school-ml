...
::: {.smaller}
1. Open **CVAT** at the address on the board → **Create an account**
2. Download **`images.zip`** and **`labels.json`** from the link on the board
...
4. In the project: **+ → Create a new task** · give it a name · drop `images.zip` into
   **Select files** · **Submit & Open**
...
::: {.notes}
Put CVAT's address and the download link on the board, as a QR code if you can. The two
files are in `temp/annotation/hands-on/`: `images.zip` holds only the ten photographs, not
the demo rosary.
