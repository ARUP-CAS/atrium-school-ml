...
# PAS (FISH object types) -> AMČR-PAS SKOS mapping; counts are validated rows only.
skos <- readr::read_csv("data/pas2amcr_summary.csv", show_col_types = FALSE) |>
  (\(d) setNames(as.list(d$value), d$key))()
...
```{.json filename="amcr-pas_v0.json"}
{"id": 9872, "image_id": 8987, "category_id": 58,
 "bbox": [837.8, 1760.0, 485.2, 445.9],
 "segmentation": [[1301.3, 1762.7, 1279.0, 1760.0, …]]}
```
...
::: {.address}
[[http://]{.address-scheme}172.16.16.214:8081](http://172.16.16.214:8081/){.external}
:::
...
::: {.figure .figure-xl}
![](img/skos-crosswalk.png){fig-alt="Eight Portable Antiquities Scheme object types joined to AMČR-PAS categories by SKOS relations: Brooch exactMatch fibula, Strap end exactMatch mordant, Harness pendant broadMatch harness component; Coin exactMatch, Token closeMatch, Coin hoard narrowMatch and Jetton relatedMatch all point at the one AMČR-PAS category coin; Seal has no AMČR-PAS category."}
:::

::: {.smaller}
The British PAS (FISH vocabulary) against AMČR-PAS: **`r skos$pas_terms` terms, and
four different ways to land on *coin***. Strap end is *mordant* — no string match finds that.
:::

::: {.notes}
Real rows from the PAS → AMČR-PAS mapping sheet, all of them marked validated.
Record counts are PAS database records for that object type. Walk down the coin box:
the same target, four relations, four different things you may do with the data —
merge coin, merge token with a note, do not relabel a hoard as *coin* (that rolls
down — next slide), keep jettons out. Seal has 16,771 PAS records and nowhere to go.
:::
...
|---|---|---|---:|
| `skos:exactMatch` | interchangeable in **any** context | merge the two classes | `r skos$exactMatch` |
| `skos:closeMatch` | interchangeable in **some** contexts | merge, and say so in the paper | `r skos$closeMatch` |
| `skos:broadMatch` / `narrowMatch` | one is **wider** than the other | roll the narrow one up; never down | `r skos$broadMatch` / `r skos$narrowMatch` |
| `skos:relatedMatch` | associated, **not** substitutable | do not merge | `r skos$relatedMatch` |
| *(none)* | no counterpart | leave out, and count what you lost | `r skos$none` |
:::
...
The PAS → AMČR-PAS column counts only the rows someone has validated.
:::
...
fish:95426   skos:prefLabel    "Jetton"@en ;
             skos:relatedMatch amcr:coin .
```

::: {.fragment}
::: {.smaller}
- One row of the sheet is **one triple** — Turtle, the W3C text format for RDF
- `fish:` URIs are the FISH thesaurus' own; `amcr:` is a **placeholder** — the sheet has AMČR-PAS labels, not URIs
- *validated* has no SKOS predicate: `r skos$unvalidated` of `r skos$pas_terms` rows are unchecked, so publish the checked ones, or say who checked what
:::
:::

::: {.notes}
Four rows straight from the mapping sheet (`fish` column → subject, `mappingRelation`
→ predicate, `target` → object). Point out that the file is readable without any
tooling — that is the argument for publishing it next to the dataset. The real
AMČR-PAS concept URIs replace the placeholder once the categories are published.
:::
...
You will lose resolution. Of the `r skos$amcr_used` AMČR-PAS categories in the dataset,
only **`r skos$amcr_used_exact`** have a validated `exactMatch` in PAS — and `r skos$into_instrument`
PAS types roll up into *instrument* alone. That is still a far better dataset than either
alone — and the mapping is a research output in its own right.
:::
:::
